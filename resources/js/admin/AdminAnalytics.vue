            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="card bg-base-100 shadow-sm">
                    <div class="card-body">
                        ...
                        <div v-if="Object.keys(data.abandon_by_question ?? {}).length"
                            class="space-y-1.5 max-h-64 overflow-y-auto">
                            <div v-for="(count, q) in data.abandon_by_question" :key="q"
                                ...
                                <div class="flex-1 bg-base-200 rounded-full h-2">
                                    <div class="bg-amber-500 h-2 rounded-full"
                                        :style="`width:${(count / maxVal(data.abandon_by_question) * 100).toFixed(1)}%`">
                                    </div>
                                </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="divide-y divide-base-200 w-full">
                            <div v-for="(count, device) in data.by_device" :key="device"
                                ...
                                <span class="font-semibold text-sm tabular-nums">{{ count?.toLocaleString('fr-CH')
                                }}</span>
                            </div>
                        </div>
                    </div>
