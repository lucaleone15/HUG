...
<template>
    <div class="max-w-xl">
        ...
        <template v-else>
            ...
            <!-- wheel pour la molette de souris -->
            <!-- .blur() sur l'input pour défocaliser et éviter des bugs -->
                ...
                <div class="card-body gap-4">
                    ...
                    <form class="grid grid-cols-1 gap-3" @submit.prevent="save">
                        <label class="form-control">
                            <div class="label"><span class="label-text">{{ t('admin.donations_count_label') }}</span>
                            </div>
                            <input v-model.number="form.donations_count" type="number" min="0" required
                                class="input input-bordered input-sm"
                                :class="fieldError('donations_count') ? 'input-error' : ''"
                        ...
                        <label class="form-control">
                            ...
                            <input v-model.number="form.lives_saved" type="number" min="0" required
                                class="input input-bordered input-sm"
                                :class="fieldError('lives_saved') ? 'input-error' : ''" @wheel="$event.target.blur()">
                            <div v-if="fieldError('lives_saved')" class="label">
                                <span class="label-text-alt text-error">{{ fieldError('lives_saved') }}</span>
                            </div>
                        </label>
                        <label class="form-control">
                            <div class="label"><span class="label-text">{{ t('admin.hug_hospitals_label') }}</span>
                            </div>
                            <input v-model.number="form.hug_hospitals_count" type="number" min="0" required
                                class="input input-bordered input-sm"
                                :class="fieldError('hug_hospitals_count') ? 'input-error' : ''"
                                @wheel="$event.target.blur()">
