    ...
    <div class="drawer lg:drawer-open min-h-screen bg-base-200">
        ...
        <div class="drawer-content flex flex-col">
            ...
            <header class="bg-base-100 border-b border-base-200 sticky top-0 z-10 px-4 h-14 flex items-center gap-3">
                ...
                <!-- Selection pour les années -->
                <button
                    class="flex items-center gap-1.5 bg-brand text-white rounded-lg px-4 py-2 text-sm font-semibold hover:bg-brand-dark transition-colors">
                    2026
        ...
        <div class="drawer-side z-20">
            ...
            <nav class="flex flex-col bg-brand w-64 min-h-full">
                ...
                <ul class="flex-1 p-3 flex flex-col gap-0.5">
                    <li v-for="item in nav" :key="item.to">
                        <RouterLink :to="item.to"
                            class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors" :class="isActive(item.to)
                ...
                <!-- Section dédiée aux réglages -->
                <div class="px-3 pb-1">
                    <div class="border-t border-white/10 pt-2">
                        <RouterLink to="/admin/settings"
                            class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-colors" :class="isActive('/admin/settings')
                                ? 'bg-white/25 text-white font-semibold'
                                : 'text-white/90 hover:text-white hover:bg-white/10'
                                " @click="closeDrawer">
                            <svg class="w-5 h-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <div class="p-3 border-t border-white/10">
                    <div class="flex items-center gap-3">
                        <div
                            class="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center text-white font-bold text-sm shrink-0">
                            {{ userInitial }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <p class="text-white text-sm font-medium truncate">{{ userName }}</p>
                        </div>
                        <button @click="logout" class="p-1.5 text-white/60 hover:text-white rounded transition-colors"
