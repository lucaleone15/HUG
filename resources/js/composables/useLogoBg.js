...
export function useLogoBg(getLogoUrl, getPrimaryColor) {
    ...
    watchEffect(() => {
        ...
        // Fond provisoire pendant la détection
        bg.value = '#f0f0f0'
...
/** SVG — fetch source + parse fills */
async function detectSvg(url) {
    const res  = await fetch(url, { cache: 'force-cache' })
    const text = await res.text()

    // Hex fills : #fff, #000, #aabbcc…
    ...
    // Pas de fills parsées → fond neutre par défaut
