...
const counters = computed(() => [
    {
        value: props.stats?.donations_count ?? 0,
        label: t("home.stats_donations"),
    },
    { value: props.stats?.lives_saved ?? 0, label: t("home.stats_lives") },
    {
        value: props.stats?.hug_hospitals_count ?? 0,
        label: t("home.stats_hospitals"),
    },
...
<template>
    <div class="min-h-screen bg-base-100 flex flex-col overflow-x-hidden">
        ...
        <!-- Hero -->
        <section
            class="relative min-h-[100dvh] flex flex-col justify-center px-6 py-20 overflow-hidden"
        >
            <!-- Image -->
            ...
            <div class="relative z-10 max-w-5xl mx-auto w-full">
                <span
                    class="h-badge inline-block text-xs tracking-[0.06em] text-white/65 mb-8"
                >
                    {{
                        t("home.edition_badge", {
                            year: new Date().getFullYear(),
                        })
                    }}
                </span>
                <h1
                    class="font-extrabold leading-[1.0] tracking-tight text-white mb-7"
                    style="font-size: clamp(2rem, 6vw, 4.5rem)"
                >
                    <span class="h-line1 block">{{
                        t("home.hero_line1")
                    }}</span>
                    <span class="h-line2 block"
                        ><template v-if="t('home.hero_line2')"
                            >{{ t("home.hero_line2") }} </template
                        ><span class="h-highlight text-brand">{{
                            t("home.hero_highlight")
                        }}</span></span
                    >
                </h1>
                <p
                    class="h-sub text-white/55 mb-10 leading-relaxed"
                    style="
        ...
        <!-- Stats Marque -->
        <div
            class="bg-brand py-5 overflow-hidden marquee-container"
            :aria-label="t('home.stats_aria')"
        >
            <div class="marquee-track flex items-baseline whitespace-nowrap">
                <template v-for="(item, i) in marqueeItems" :key="`m-${i}`">
                    <span
                        class="text-white font-extrabold tabular-nums shrink-0 mx-4 leading-none"
                    ...
                    <span
                        class="text-white/75 tracking-[0.04em] shrink-0 mx-4"
                        style="font-size: clamp(0.8rem, 1.2vw, 0.9rem)"
                    >
                        {{ item.label }}
                    </span>
                    <span
                        class="text-white/50 shrink-0 mx-3 leading-none"
                        style="font-size: clamp(1.75rem, 3vw, 2.5rem)"
                        aria-hidden="true"
                        >·</span
                    >
        ...
        <section class="py-20 px-6 bg-base-100" ref="progRef">
            <div class="max-w-5xl mx-auto">
                <h2
                    class="font-bold mb-8 reveal-up"
                ...
                <div class="flex flex-col gap-3">
                    <div
                        class="reveal-up prog-row-wrap"
                        :class="{ 'reveal-up--visible': progVisible }"
                    >
                        <ProgrammeRow
                            :title="t('trophee.title')"
                            :description="t('trophee.subtitle')"
                            href="/trophee"
                        >
                            <template #visual>
                                <div
        ...
        <!-- CTA -->
        <section
            class="py-24 px-6 bg-brand text-white relative overflow-hidden"
            ref="ctaRef"
        >
            <div
                class="absolute inset-0 cta-dot-grid pointer-events-none"
                aria-hidden="true"
            ></div>
            <div
                class="max-w-5xl mx-auto relative z-10 grid md:grid-cols-2 gap-12 items-center"
            >
                <div
                    class="reveal-up"
                ...
                >
                    ...
                    <p
                        class="text-white/65 mb-10 leading-relaxed"
                ...
                <div
                    class="reveal-up aspect-[4/3] rounded-xl overflow-hidden select-none"
                    :class="{ 'reveal-up--visible': ctaVisible }"
                    style="
                        transition-delay: 160ms;
                        background-image: url(&quot;https://images.unsplash.com/photo-1697192156499-d85cfe1452c0?auto=format&fit=crop&w=900&q=80&quot;);
...
.h-badge {
    animation: h-rise 420ms cubic-bezier(0.23, 1, 0.32, 1) both;
    animation-delay: 0ms;
}
.h-line1 {
    animation: h-rise 520ms cubic-bezier(0.23, 1, 0.32, 1) both;
...
.h-sub {
    animation: h-rise 480ms cubic-bezier(0.23, 1, 0.32, 1) both;
...
.prog-row-wrap {
    ...
    padding: 0 1.5rem;
    box-shadow:
        0 1px 3px rgba(25, 5, 7, 0.08),
        0 1px 2px rgba(25, 5, 7, 0.06);
    transition:
...
@media (hover: hover) and (pointer: fine) {
    .prog-row-wrap:hover {
        box-shadow:
            0 10px 30px rgba(25, 5, 7, 0.12),
            0 4px 8px rgba(25, 5, 7, 0.08);
