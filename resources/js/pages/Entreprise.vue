...
<template>
    <div class="min-h-screen bg-base-100 flex flex-col overflow-x-hidden" :style="`--c1: ${c1}; --c2: ${c2}; --t1: ${t1}; --t2: ${t2}`">
        ...
        <!-- Hero -->
        <section class="relative overflow-hidden min-h-[60vh] md:min-h-[95vh]" style="background-color: var(--c1);">
            <!-- Inspecteur — ancré à droite pour toujours être visible -->
            <svg aria-hidden="true"
                 class="absolute inset-0 w-full h-full pointer-events-none entreprise-hero-img"
                 viewBox="0 0 1920 1128" preserveAspectRatio="xMaxYMid slice"
                 xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#insp-clip)">
                    <path d="M933.799 1499.7C933.799 1499.7 950.576 1006.89 1025.04 933.053C1099.51 859.21 1143.94 732.146 1386.42 639.995C1628.03 548.139 1645.24 571.295 1645.24 571.295C1645.24 571.295 1780.06 580.492 1846.21 595.391C1912.24 610.286 2303.63 748.876 2396.36 872.46C2396.36 872.46 2332.11 980.342 1963.5 1160C1647.77 1313.88 1011.01 1450.8 933.799 1499.7Z" fill="#E62F2E"/>
                    <path d="M812.229 1028.01L974.547 870.144L935.954 369.91L1163.98 319.895L1393.4 63.9816L1839.23 502.317L2101.3 503.105L2117 536.8L812.229 1028.01Z" fill="#111111"/>
                    <path d="M999.017 993.27L938.056 1087.17L1506.92 972.352C1518.22 970.051 1529.09 965.871 1538.95 959.969L2145.17 598.385L2054.82 583.075L999.017 993.27Z" fill="#111111"/>
                    <path d="M1225.66 939.31L1508.32 844.193C1508.32 844.193 1503.03 946.769 1409.08 975.177C1315.12 1003.58 1289.48 985.623 1225.66 939.31Z" fill="white"/>
                    <path d="M1522.23 840.268C1522.23 840.268 1600.84 901.792 1668.87 868.171C1736.88 834.506 1775.54 714.934 1775.54 714.934L1515.2 843.755" fill="white"/>
                </g>
                <defs>
...
<div class="relative z-10 max-w-5xl mx-auto py-12 md:py-20 px-4 md:px-6 flex items-center min-h-[60vh] md:min-h-[95vh]">
                <div class="page-hero-text hero-text-bg w-full md:w-1/2 text-center md:text-left">
        ...
        <!-- Countdown + Collectes — fond neutre, accents c1 -->
        <section class="px-4 md:px-6 bg-base-100" :class="hasCountdown || upcomingCollectes.length ? 'py-10 md:py-14' : 'py-6 md:py-8'">
            <div class="max-w-5xl mx-auto">

                <!-- Countdown -->
                <div v-if="hasCountdown" class="mb-8 pb-8 border-b border-base-200">
                    <p class="text-sm font-semibold mb-4 text-base-content/60">{{ t('entreprise.countdown_label') }}</p>
                    <div class="flex items-end gap-3">
                        <div class="flex-1 text-center">
                            <div class="font-extrabold tabular-nums leading-none" style="color: var(--c1); font-size: clamp(2rem, 5vw, 3rem);">{{ pad(countdown.days) }}</div>
                            <div class="text-sm mt-2 text-base-content/60">{{ t('entreprise.countdown_days') }}</div>
                        </div>
                        <span class="pb-5 font-bold shrink-0 text-xl text-base-content/20">:</span>
                        <div class="flex-1 text-center">
                            <div class="font-extrabold tabular-nums leading-none" style="color: var(--c1); font-size: clamp(2rem, 5vw, 3rem);">{{ pad(countdown.hours) }}</div>
                            <div class="text-sm mt-2 text-base-content/60">{{ t('entreprise.countdown_hours') }}</div>
                        </div>
                        <span class="pb-5 font-bold shrink-0 text-xl text-base-content/20">:</span>
                        <div class="flex-1 text-center">
                            <div class="font-extrabold tabular-nums leading-none" style="color: var(--c1); font-size: clamp(2rem, 5vw, 3rem);">{{ pad(countdown.minutes) }}</div>
                            <div class="text-sm mt-2 text-base-content/60">{{ t('entreprise.countdown_minutes') }}</div>
                        </div>
                    </div>
                    <p v-if="nextCollecte?.rdv_date" class="text-sm mt-4 text-base-content/60">
                        {{ t('entreprise.collect_date') }} : <strong class="text-base-content">{{ fullDate(nextCollecte.rdv_date) }}</strong>
                    </p>
                </div>
                <div v-else>
                    <p class="text-sm font-semibold mb-2 text-base-content/60">{{ t('entreprise.collecte_none_label') }}</p>
                    <p class="font-bold mb-1 text-base-content" style="font-size: clamp(1rem, 2vw, 1.2rem);">{{ t('entreprise.collecte_none_title') }}</p>
                    <p class="text-sm text-base-content/60" style="max-width: 38ch;">{{ t('entreprise.collecte_none_description', { company: entreprise.name }) }}</p>
                </div>
                        ...
                        <a v-for="c in upcomingCollectes" :key="c.id"
                           ...
                           style="background-color: var(--c1);">
                            <div class="shrink-0 rounded-lg w-14 text-center py-2"
                                 style="background-color: color-mix(in srgb, var(--t1) 15%, transparent); color: var(--t1);">
        ...
        <!-- Stats — fond de marque -->
        <section class="px-4 md:px-6 py-10 md:py-14" style="background-color: var(--c1);" ref="statsRef">
            <div class="max-w-5xl mx-auto">
                <div class="reveal-up" :class="{ 'reveal-up--visible': statsVisible }">
                    <p class="text-sm font-semibold mb-5" style="color: var(--t1); opacity: 0.72;">{{ t('entreprise.stats_label') }}</p>
                    <div class="flex flex-wrap gap-6 gap-x-12">
                        <div v-if="entreprise.eligible_count" style="transition-delay: 80ms;">
                            <div class="font-bold leading-none mb-1" style="color: var(--t1); font-size: clamp(2rem, 5vw, 3rem);">{{ entreprise.eligible_count.toLocaleString() }}</div>
                            <div class="text-sm" style="color: var(--t1); opacity: 0.72;">{{ t('home.stats_eligible') }}</div>
                        </div>
                        <div v-if="entreprise.submissions_count" style="transition-delay: 160ms;">
                            <div class="font-bold leading-none mb-1" style="color: var(--t1); font-size: clamp(2rem, 5vw, 3rem);">{{ entreprise.submissions_count.toLocaleString() }}</div>
                            <div class="text-sm" style="color: var(--t1); opacity: 0.72;">{{ t('entreprise.stats_quiz') }}</div>
                        </div>
                        <div v-if="entreprise.employee_count" style="transition-delay: 240ms;">
                            <div class="font-bold leading-none mb-1" style="color: var(--t1); font-size: clamp(2rem, 5vw, 3rem);">{{ entreprise.employee_count.toLocaleString() }}</div>
                            <div class="text-sm" style="color: var(--t1); opacity: 0.72;">{{ t('entreprise.employees') }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
@media (prefers-reduced-motion: reduce) {
    .entreprise-hero-img { animation: none; }
}
