...
defineProps({
    winner: {
        type: Object,
        required: true,
        // { id, name, trophy_rank, logo_url, primary_color }
    },
...
<template>
    ...
    >
        <div class="card-body items-center">
            ...
            <h2 class="card-title text-base justify-center">{{ winner.name }}</h2>
    ...
    <li v-else class="flex items-center gap-4 py-3">
        <span class="text-2xl w-8 text-center">{{ medal(winner.trophy_rank) }}</span>
        <div class="w-1 h-8 rounded-full" :style="`background-color: ${winner.primary_color}`"></div>
        <span class="font-medium">{{ winner.name }}</span>
        <span class="text-xs text-base-content/50 ml-auto">#{{ winner.trophy_rank }}</span>
    </li>
</template>
