...
<template>
    <a :href="href" class="row group flex flex-col md:flex-row md:items-center gap-3 md:gap-6 py-9">
        ...
        <div class="flex items-start justify-between gap-4 flex-1 min-w-0">
            <div class="min-w-0">
                <h3 class="title font-bold mb-2" style="font-size: clamp(1.1rem, 2vw, 1.4rem);">
                    {{ title }}
                </h3>
                <p v-if="description" class="text-base-content/55 text-sm leading-relaxed" style="max-width: 58ch;">
                    {{ description }}
                </p>
            </div>
            <!-- gestion flèche sur mobile -->
